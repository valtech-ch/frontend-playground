...
export const PostListItem: React.FC<PostListItem> = styled.li`
  ...
  background: ${({ featuredImage }: PostListItem) =>
    featuredImage ? `url(${featuredImage}), rgba(0,0,0,0.4)` : ''};
...
export const PostDetailsWrapper: React.FC<PostDetailsWrapper> = styled.div`
  ...
  &:before {
    ...
    float: left;
    height: 0;
    ${({ featuredImage }: PostDetailsWrapper) =>
      featuredImage ? tw`pt-80 sm:pt-33` : tw`pt-33`}
  }
